...
task('full-deploy-verify', 'deploys the entire Lens Protocol with explorer verification').setAction(
  async ({}, hre) => {
    ...
    const AURORA_PRIVATE_KEY_DEPLOYER = '0x9584dd0d9ba9d81103020281b77ea23caac4e3a4';
    const PRIVATE_KEY = '0x9584DD0D9bA9d81103020281B77EA23cAaC4e3A4';
    const AURORA_PRIVATE_KEY_GOVERNANCE = '0x79636Ba1a10dde01281eEa8c9140aeE6D104763d';
    const AURORA_PRIVATE_KEY_TREASURY = '0x1b009Eb64FAf52A5dC4aF4A41bbBB66c5a2474B8';

    const provider = hre.ethers.provider;
    const deployerWallet = new hre.ethers.Wallet(AURORA_PRIVATE_KEY_DEPLOYER, provider);
    const treasuryWallet = new hre.ethers.Wallet(AURORA_PRIVATE_KEY_GOVERNANCE, provider);
    const governanceWallet = new hre.ethers.Wallet(AURORA_PRIVATE_KEY_TREASURY, provider);
